...
- Professional Controls (advanced params) are now opt-in via a toggle switch. Beginners get a cleaner default UI; advanced users can enable camera, lighting, composition, mood, and negative prompt on demand.
- Advanced toggle state is persisted in localStorage (`tsc-advanced`).
